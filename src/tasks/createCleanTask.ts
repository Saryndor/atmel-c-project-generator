...
/**
 * Creates a programmatic task for building the firmware.
 * This defines WHAT the task does (run 'make clean'), but not HOW it is triggered.
 ...
 */
export function createCleanTask(projectRoot: vscode.WorkspaceFolder): vscode.Task {
    
    // 1. Define the command execution
    // We execute "make clean" in the root of the provided workspace folder.
    const execution = new vscode.ShellExecution("make clean", {
    ...
    const task = new vscode.Task(
        { type: 'atmel-task' },
        projectRoot,                // Scope
        "Clean .build Folder",      // Name (visible in terminal title)
