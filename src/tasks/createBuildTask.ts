 ...
 * This defines WHAT the task does (run 'make all'), but not HOW it is triggered.
 ...
 */
export function createBuildTask(projectRoot: vscode.WorkspaceFolder): vscode.Task {
    
    // 1. Define the command execution
    // We execute "make all" in the root of the provided workspace folder.
    const execution = new vscode.ShellExecution("make all", {
    ...
    const task = new vscode.Task(
        { type: 'atmel-task' },
        projectRoot,                // Scope
        "Build Firmware",           // Name (visible in terminal title)
